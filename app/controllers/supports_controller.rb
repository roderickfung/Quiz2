class SupportsController < ApplicationController
  ...
  def index
    ...
    @limit ||= 7
    @query = params[:query]
    @search = Support.search(params[:query])
    @supports = @search.order(complete: :asc, id: :desc).page(params[:page]).per(@limit)
    @change = params[:complete]
  ...
  private
    ...
    def support_params
      params.require(:support).permit(:name, :email, :department, :message, :complete)
    end
end
